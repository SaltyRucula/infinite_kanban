...
| `JIRA_BASE_URL` | _(unset)_ | Server-only Jira base URL used by `/api/jira/import-assigned` (for example `https://jira.example.com`) |
| `JIRA_USER_EMAIL` | _(unset)_ | Server-only Jira user email required for cloud/basic auth mode |
| `JIRA_API_TOKEN` | _(unset)_ | Server-only Jira API token used only in Authorization headers to Jira |
| `JIRA_IS_DATACENTER` | `false` | When `true`, Jira import uses `Bearer <token>` (Data Center mode); otherwise uses Basic `email:token` |
...
`projects:read`, `agents:read`, `orchestrations:create`, `orchestrations:read`,
`orchestrations:message`, and `jira:import`. They cannot merge, create PRs, delete resources, or mutate
projects. `API_KEY` remains the legacy full-access credential for the UI.

### Jira assigned-issue import

`POST /api/jira/import-assigned` is a **read-only Jira importer**. It never writes back
to Jira. The server requests assigned issues with Data Center-compatible
`POST /rest/api/2/search` (`assignee = currentUser() AND statusCategory in ("To Do", "In Progress") ORDER BY updated DESC`),
maps them into backlog tasks, and stores them idempotently using
`externalSource: "jira"` + a stable external key based on normalized Jira base URL
and Jira issue id.

- Missing Jira env config returns `503` with a clear message.
- Jira auth/upstream failures return safe `502` responses (no token leakage).
- Only issues in Jira's `To Do` or `In Progress` status categories are imported; closed/done issues are excluded.
- Re-importing skips existing Jira-backed tasks and does not overwrite prior imports.

Scheduled imports are configured per project in **Edit Project**. They are disabled by default and use a 15-minute interval when enabled; valid intervals are 5–1,440 minutes. The **Import Jira** button remains available for a run-now import regardless of the schedule setting. Scheduled and manual imports share the same per-project overlap lock, persist last-run counts/errors, and resume after restart in the single server process.
...
| `OPENCODE_BASE_URL` | _(unset)_ | Optional server-side OpenCode endpoint. When set, Agent Board connects to that existing OpenCode server instead of spawning a managed one. Must be loopback (`localhost`, `127.0.0.1`, or `[::1]`) with an explicit port and no path/query/hash/userinfo. |
...
The repo ships generic launchd templates and wrappers in `scripts/launchd/`.
Install them into `~/Library/LaunchAgents` with a private env file outside the repo:

```bash
scripts/launchd/manage-launchd.sh install --private-env-file "$HOME/.config/ai-agent-board/private.env"
scripts/launchd/manage-launchd.sh status
scripts/launchd/manage-launchd.sh logs
scripts/launchd/manage-launchd.sh unload
```

The private env file must exist, be owned by your user, and stay private (no group/world access). If it defines `OPENCODE_BASE_URL`, the value must be loopback with an explicit port and no userinfo/path/query/hash, using either `http://` or `https://` (for example: `http://localhost:<port>`, `https://localhost:<port>`, `http://127.0.0.1:<port>`, `https://127.0.0.1:<port>`, `http://[::1]:<port>`, or `https://[::1]:<port>`). The board server example remains `OPENCODE_BASE_URL=http://127.0.0.1:4098` for local OpenCode connection — deliberately not port 4096, which is the OpenCode CLI's own default/auto-attach port and may already be in use by an interactive `opencode` session on the same machine. The Vite wrapper rejects public `HOST` / `VITE_ALLOWED_HOSTS` values from the private env. To start the services after install, use the printed `launchctl bootstrap gui/$(id -u) ...` commands for the server, client, and OpenCode plists.
