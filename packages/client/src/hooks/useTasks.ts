...
export function useTasks(projectId = 'default') {
  ...
  const resumeClarification = useCallback(async (
    id: string,
    data: ResumeClarificationRequest,
  ): Promise<ResumeClarificationResponse | undefined> => {
    try {
      return await api.resumeClarification(id, data);
    } catch (err) {
      setError(`Failed to submit clarification: ${(err as Error).message}`);
      return undefined;
    }
  }, []);
  ...
  const openOpenCodeSession = useCallback(async (id: string) => {
    try {
      const { url } = await api.getOpenCodeSessionUrl(id);
      window.open(url, '_blank', 'noopener,noreferrer');
    } catch (err) {
      setError(`Failed to open OpenCode session: ${(err as Error).message}`);
    }
  }, []);
