...
interface RenderClarificationRequest {
  requestId: string;
  prompt: string;
...
function parseClarificationRequestPayload(
  ...
  if (!requestId || !prompt || !Number.isFinite(timestamp)) return null;

  const rawChoices = Array.isArray(value.choices) ? value.choices : [];
  const choices = rawChoices.filter((choice): choice is string => typeof choice === 'string' && choice.length > 0);

  const maybeSessionId = (value as TaskClarificationRequest).sessionId;
  const sessionId = typeof maybeSessionId === 'string' && maybeSessionId.trim().length > 0
    ? maybeSessionId.trim()
    : null;
  ...
  return {
    ...
    timestamp,
    sessionId,
    source: sessionId ? 'task' : 'event',
  };
}

function isAnswerForRequest(answer: TaskClarificationAnswer | null | undefined, requestId: string): answer is TaskClarificationAnswer {
  if (!answer) return false;
  return typeof answer.requestId === 'string' && answer.requestId === requestId && typeof answer.answer === 'string';
}

function parseClarificationAnswer(
  value: TaskClarificationAnswer | null | undefined,
): TaskClarificationAnswer | null {
  if (!value) return null;
  const requestId = typeof value.requestId === 'string' ? value.requestId.trim() : '';
  const answer = typeof value.answer === 'string' ? value.answer.trim() : '';
  const timestamp = typeof value.timestamp === 'number' ? value.timestamp : Number.NaN;
  const sessionId = typeof value.sessionId === 'string' ? value.sessionId.trim() : '';
  if (!requestId || !answer || !sessionId || !Number.isFinite(timestamp)) return null;
  return { requestId, answer, timestamp, sessionId };
}
...
interface AgentPanelProps {
  ...
  onResumeClarification?: (id: string, data: ResumeClarificationRequest) => Promise<ResumeClarificationResponse | undefined>;
...
export function AgentPanel({ task, onClose, onRun, onStop, onResumeClarification, onCreatePR, onMergeLocal, onCleanupWorktree, onReconfigureRetry, theme }: AgentPanelProps) {
  ...
  const latestEventClarificationRequest = useMemo(() => {
    for (let i = events.length - 1; i >= 0; i -= 1) {
      const parsed = parseClarificationRequestPayload(events[i].metadata?.clarification_request);
      if (parsed) return parsed;
    }
    return null;
  }, [events]);

  const activeClarificationRequest = useMemo(() => {
    if (task?.agentStatus === 'awaiting_clarification' && taskClarificationRequest) {
      return taskClarificationRequest;
    }

    const candidates: RenderClarificationRequest[] = [taskClarificationRequest, latestEventClarificationRequest]
      .filter((candidate): candidate is RenderClarificationRequest => candidate !== null);
    ...
    if (!latest.sessionId && taskClarificationRequest?.requestId === latest.requestId && taskClarificationRequest.sessionId) {
      return { ...latest, sessionId: taskClarificationRequest.sessionId };
    }

    return latest;
  }, [task?.agentStatus, taskClarificationRequest, latestEventClarificationRequest]);

  const taskClarificationAnswer = useMemo(
    () => parseClarificationAnswer(task?.clarificationAnswer),
    [task?.clarificationAnswer],
  );

  const latestEventClarificationAnswer = useMemo(() => {
    for (let i = events.length - 1; i >= 0; i -= 1) {
      const parsed = parseClarificationAnswer(events[i].metadata?.clarification_answer);
      if (parsed) return parsed;
    }
    return null;
  }, [events]);

  const activeClarificationAnswer = useMemo(() => {
    if (!activeClarificationRequest) return null;
    const candidates: TaskClarificationAnswer[] = [
      taskClarificationAnswer,
      latestEventClarificationAnswer,
      localClarificationAnswer,
    ].filter(
      (candidate): candidate is TaskClarificationAnswer =>
        isAnswerForRequest(candidate, activeClarificationRequest.requestId),
    );
    if (candidates.length === 0) return null;
    return candidates.reduce((current, candidate) =>
      candidate.timestamp >= current.timestamp ? candidate : current,
    );
  }, [activeClarificationRequest, taskClarificationAnswer, latestEventClarificationAnswer, localClarificationAnswer]);

  const showClarificationCard = useMemo(() => {
    if (!activeClarificationRequest) return task?.agentStatus === 'awaiting_clarification';
    return (
      task?.agentStatus === 'awaiting_clarification'
      || latestEventClarificationRequest !== null
      || taskClarificationRequest !== null
    );
  }, [
    activeClarificationRequest,
    task?.agentStatus,
    latestEventClarificationRequest,
    taskClarificationRequest,
  ]);
  ...
  useEffect(() => {
    ...
    setClarificationError(null);
    setLocalClarificationAnswer((prev) =>
      prev && prev.requestId === activeClarificationRequest.requestId ? prev : null,
    );
  ...
  const submitClarification = async (rawAnswer: string) => {
    if (!task || !activeClarificationRequest || clarificationSubmitting || clarificationSubmitInFlightRef.current) return;
    if (activeClarificationAnswer) return;
    if (!clarificationAcceptingResponses) {
      setClarificationError('This clarification request no longer accepts responses because the task is no longer awaiting clarification.');
      return;
    }

    const answer = rawAnswer.trim();
    if (!answer) {
      setClarificationError('Clarification answer cannot be empty.');
      return;
    }

    if (!activeClarificationRequest.sessionId) {
      setClarificationError('Cannot submit clarification: missing sessionId for this request.');
      return;
    }
    ...
    setClarificationError(null);
    try {
      const payload: ResumeClarificationRequest = {
        requestId: activeClarificationRequest.requestId,
        sessionId: activeClarificationRequest.sessionId,
        answer,
      };

      const response = onResumeClarification
        ? await onResumeClarification(task.id, payload)
        : await api.resumeClarification(task.id, payload);
      if (!response?.success) {
        setClarificationError('Failed to submit clarification.');
        return;
      }

      setLocalClarificationAnswer({
  ...
  const clarificationInputDisabled =
    !activeClarificationRequest
    || !clarificationAcceptingResponses
              ...
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs font-semibold text-amber-700 dark:text-amber-300">Clarification required</p>
                    <p data-testid="clarification-prompt" className="mt-1 whitespace-pre-wrap text-xs leading-relaxed text-amber-700/85 dark:text-amber-300/85">
                      {activeClarificationRequest?.prompt ?? 'Awaiting clarification payload from task state/events.'}
                    </p>
                    {activeClarificationRequest && (
                      <p className="mt-1 font-mono text-[10px] text-amber-700/70 dark:text-amber-300/70">
                        requestId: {activeClarificationRequest.requestId}
                        {activeClarificationRequest.sessionId ? ` · sessionId: ${activeClarificationRequest.sessionId}` : ' · sessionId: missing'}
                      </p>
                    )}
                  </div>
                  {activeClarificationRequest?.prompt && <CopyButton text={activeClarificationRequest.prompt} />}
                </div>

                {activeClarificationRequest?.choices.length ? (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {activeClarificationRequest.choices.map((choice) => (
                      <button
                        key={choice}
                        type="button"
                        onClick={() => submitClarification(choice)}
                        disabled={clarificationInputDisabled}
                        className="rounded-md border border-amber-500/30 bg-card px-2.5 py-1 text-xs text-foreground hover:bg-amber-500/15 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {choice}
                      </button>
                    ))}
                ...
                {activeClarificationAnswer ? (
                  <div className="mt-3 rounded-md border border-emerald-500/25 bg-emerald-500/10 px-2.5 py-2 text-xs text-emerald-700 dark:text-emerald-300">
                    Submitted: {activeClarificationAnswer.answer}
                  </div>
                ) : (
                    ...
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' && !e.shiftKey) {
                          e.preventDefault();
                          void submitClarification(clarificationInput);
                        }
                      }}
                      placeholder="Reply to clarification..."
                      disabled={clarificationInputDisabled}
                      className="flex-1 rounded-md border border-amber-500/25 bg-card px-3 py-1.5 text-xs text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-1 focus:ring-amber-500 disabled:opacity-50 disabled:cursor-not-allowed"
                    />
                    <button
                      type="button"
                      onClick={() => void submitClarification(clarificationInput)}
                      disabled={clarificationInputDisabled || !clarificationInput.trim()}
                      className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-amber-500/25 bg-card text-amber-600 dark:text-amber-300 hover:bg-amber-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      title="Submit clarification reply"
                    >
                ...
                {clarificationSubmitting && (
                  <p className="mt-2 text-[11px] text-amber-700/80 dark:text-amber-300/80">Submitting clarification…</p>
                )}
                {clarificationError && (
                  <p className="mt-2 whitespace-pre-wrap text-[11px] text-red-700 dark:text-red-300">{clarificationError}</p>
                )}
              </div>
            </div>
          )}
