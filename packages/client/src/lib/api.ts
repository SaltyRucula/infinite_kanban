...
export interface ResumeClarificationResponse {
  success: true;
  code: string;
  message: string;
}
...
export const api = {
  ...
  resumeClarification: (id: string, data: ResumeClarificationRequest) =>
    request<ResumeClarificationResponse>(`/tasks/${id}/clarification/resume`, { method: 'POST', body: JSON.stringify(data) }),

  getOpenCodeSessionUrl: (id: string) =>
    request<{ sessionId: string; url: string }>(`/tasks/${id}/opencode-session`),
