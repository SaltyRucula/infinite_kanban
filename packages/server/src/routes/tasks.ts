...
export function createTaskRouter(repo: TaskRepository, agentManager: AgentManager, projectRepo: ProjectRepository): Router {
  ...
  router.patch('/:id', asyncHandler(async (req: Request, res: Response) => {
    ...
    if (columnId !== undefined && !isValidColumnId(columnId)) {
      res.status(400).json({ error: 'invalid columnId: must be one of backlog, in-progress, pending, review, done' });
      return;
    }
    if (agentStatus !== undefined && !isValidAgentStatus(agentStatus)) {
      res.status(400).json({ error: 'invalid agentStatus: must be one of idle, planning, executing, awaiting_clarification, complete, failed' });
      return;
    }
    ...
    if (columnId && columnId !== task.columnId) {
      if (columnId === 'pending') {
        res.status(400).json({ error: 'columnId pending is reserved for system clarification flow' });
        return;
      }
      ...
      if (task.columnId === 'pending' && columnId === 'in-progress' && agentManager.isRunning(task.id)) {
        res.status(409).json({ error: 'cannot reset pending task while agent session is running' });
        return;
      }
    }
