...
| `SERVICE_TOKENS` | _(none)_ | JSON array of scoped service credentials (`token` or preferred `sha256`) for integrations; include `jira:import` scope for `/api/jira/*` |
...
| `JIRA_BASE_URL` | _(none)_ | Server-only Jira base URL for assigned-issue import |
| `JIRA_USER_EMAIL` | _(none)_ | Server-only Jira user email for basic auth mode |
| `JIRA_API_TOKEN` | _(none)_ | Server-only Jira API token; never expose to client/logs |
| `JIRA_IS_DATACENTER` | `false` | `true` = bearer token mode (Jira Data Center); unset/false = basic email:token |

### Jira import semantics

- Endpoint: `POST /api/jira/import-assigned` (scope: `jira:import` for service tokens).
- Jira access is read-only via `POST /rest/api/2/search` with current-user JQL restricted to the `To Do` and `In Progress` status categories.
- Import is idempotent: existing `externalSource=jira` + `externalKey` rows are skipped.
- Only newly created tasks are broadcast; existing imported tasks remain unchanged.
- Scheduled imports are per-project, disabled by default, and run every 15 minutes when enabled (valid range 5–1,440 minutes). The manual button remains a run-now path; both paths share a per-project overlap lock and persisted run metadata.
